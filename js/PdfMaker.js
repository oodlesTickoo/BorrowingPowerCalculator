app.service('PdfMaker', [function() {

    this.createChart = function(normalDetails, child1Detail, child2Detail, child3Detail, child4Detail, child5Detail, child6Detail) {

        var moneyFormat = wNumb({
            mark: '.',
            thousand: ',',
            prefix: '$',
            decimals: 2
        });

        function reduceToCapitalize(nameArr) {
            return nameArr.reduce(function(first, second) {
                return first[0].toUpperCase() + first.slice(1) + " " + second[0].toUpperCase() + second.slice(1)
            })
        }



        var imgData2 = 'data:image/png;base64,';

        var columnsP1 = [
            { title: "Private School Fee Calculations", dataKey: "info" },
        ];

        var columnsP2 = [
            { title: "Comparison Graphs", dataKey: "info" },
        ];


        var columnsP3 = [{ title: "Calculator Information", dataKey: "info" }, ];


        var rowsP3 = [
            { "info": "1) It is assumed that the school fee includes both the upfront application fee as well as the ongoing annual fee." },
            { "info": "2) School fee will be indexed at the CPI level." },
            { "info": "3) It gives the user to choose from the pre-set investment portfolios or their own nominated portfolio. In the case that the user wants to use their own portfolio, they need to define the net expected rate of return of the investment portfolio." },
            { "info": "4) It is assumed that the rate of return we used in the calculation is the net portfolio return after tax and fees." },
            { "info": "5) It allows user to aggregate up to five children's data into the calculator." },
            { "info": "6) It is assumed the expected present value as at the portfolio start date is the net present value at the time the investor starts contributing to the portfolio." },
            { "info": "7) It is assumed that the investor contribute into the portfolio at the end of each year - contribution in arrear." }
        ];

        var columns1 = [
            { title: "General Information", dataKey: "info" },
            { title: "Values", dataKey: "value" },
        ];
        var rows1 = [
            { "info": "Full Name", "value": reduceToCapitalize((normalDetails.firstName.trim() + " " + normalDetails.lastName.trim()).split(' ')) },
            { "info": "E-Mail", "value": normalDetails.email.trim() },
            { "info": "Mobile Number", "value": "0" + normalDetails.mobile },
            { "info": "Initial Investment Amount", "value": moneyFormat.to(normalDetails.initialInvestmentAmount) }
        ];

        if (normalDetails.postalCode != undefined) {
            var postCode = normalDetails.postalCode;

            if (normalDetails.postalCode < 10) {
                postCode = "000" + normalDetails.postalCode
            }
            if (normalDetails.postalCode >= 10 && normalDetails.postalCode < 100) {
                postCode = "00" + normalDetails.postalCode
            }
            if (normalDetails.postalCode >= 100 && normalDetails.postalCode < 1000) {
                postCode = "0" + normalDetails.postalCode
            }
            rows1.push({ "info": "Postal Code", "value": postCode });
        }

        var options = {
            theme: 'grid'
        };

        var doc = new jsPDF('p', 'pt');

        doc.autoTable(columnsP1, [], {
            margin: { top: 20 },
            styles: {
                rowHeight: 40,
                halign: 'left',
                valign: 'middle',
                fontSize: 15
            }
        });

        doc.autoTable(columns1, rows1, {
            margin: { top: 80 },
            styles: {
                halign: "left",
                overflow: "linebreak"
            },
            columnStyles: {
                info: { columnWidth: 340 },
                value: { columnWidth: 177 }

            },
        });

        var top = doc.autoTableEndPosY();

        doc.autoTable(columnsP2, [], {
            margin: { top: top+40 },
            styles: {
                rowHeight: 30,
                halign: 'left',
                valign: 'middle',
                fontSize: 15
            }
        });

        var top = doc.autoTableEndPosY();

        var canvas = document.createElement("canvas");

        canvg(canvas, $('#containerA').highcharts().getSVG());

        var img = canvas.toDataURL("image/png");

        doc.addImage(img, 'PNG', 60, top + 40);



        doc.addImage(imgData2, 'PNG', 40, 790);
        doc.setFontSize(10);
        doc.text(510, 820, 'PAGE ' + 1);

        doc.addPage();

        

        var top = doc.autoTableEndPosY();

        var canvas = document.createElement("canvas");

        canvg(canvas, $('#container').highcharts().getSVG());

        var img = canvas.toDataURL("image/png");

        doc.addImage(img, 'PNG', 60, 20);

        doc.autoTable(columnsP3, rowsP3, {
            margin: { top: 380 },
            styles: {
                overflow: 'linebreak',
                fontSize: 10
            }
        });

        doc.addImage(imgData2, 'PNG', 40, 790);
        doc.setFontSize(10);
        doc.text(510, 820, 'PAGE ' + 2);


        doc.save('PrivateSchoolFeeCalculation.pdf');

    }
}]);
